...
namespace Live2D.Cubism.Framework.Motion
{
    ...
    public class CubismMotionController : MonoBehaviour
    {
        ...
        public void PlayAnimation(AnimationClip clip, int layerIndex = 0, int priority = CubismMotionPriority.PriorityNormal, bool isLoop = true, float speed = 1.0f)
        {
            ...
            {
                Debug.Log($"PlayAnimation failed: enabled={enabled}, _isActive={_isActive}, _cubismFadeMotionList==null:{_cubismFadeMotionList == null}, clip==null:{clip == null}, layerIndex={layerIndex}, LayerCount={LayerCount}, priority={priority}, _motionPriorities[layerIndex]={(_motionPriorities != null && layerIndex >= 0 && layerIndex < _motionPriorities.Length ? _motionPriorities[layerIndex].ToString() : "N/A")}");
